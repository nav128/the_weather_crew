...
def parse_range(payload: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
	...
	query = query.strip()
	if not query:
		return {"error": EMPTY_QUERY_ERROR, "hint": FORMAT_HINT}

	# Use the single deterministic pattern to extract location, start, end, unit
	m = PATTERN.search(query)
	if not m:
		return {"error": PARSE_ERROR, "hint": FORMAT_HINT}
	...
	if not start_dt or not end_dt or sucsses_start == 0 or sucsses_end == 0:
		return {"error": DATE_PARSE_ERROR, "hint": DATES_UNKNOWN_HINT + f"\n()"}
	...
	if end_date < start_date:
		return {"error": DATE_ORDER_ERROR, "hint": DATES_RANGE_HINT.format(raw_start=raw_start, raw_end=raw_end)}

	span_days = (end_date - start_date).days + 1
	if not 0 < span_days <= 31:
		return {"error": f"date range exceeds 31 days ({span_days} days)", "hint":  DATES_RANGE_HINT.format(raw_start=raw_start, raw_end=raw_end)}
	...
	if not COORDINATES_RE.match(location):
		for x in range(3):
			try:
				...
				if not _location:
					return {"error": lOCATION_ERROR, "hint": LOCATION_HINT.format(location=location)}
				location = f"{_location.latitude},{_location.longitude}"
				break
			except geopy.exc.GeocoderUnavailable:
				continue
		else:
			return {"error": lOCATION_ERROR, "hint": GEOCODE_SERVICE_UNAVAILABLE}
